    ...
    <section id={sectionId} className={`px-4 py-16 sm:px-6 sm:py-20 ${palette.section}`}>
      <div className={`mx-auto max-w-6xl ${isSplit ? 'grid items-center gap-12 md:grid-cols-2' : 'text-center'}`}>
        <div>
          ...
          <h1 className="mt-5 text-3xl font-bold tracking-tight text-slate-100 sm:text-4xl md:text-5xl">{title}</h1>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-slate-300 md:text-lg">{subtitle}</p>
          {ctaText && (
        ...
        {imageUrl && (
          <div className={`${isSplit ? '' : 'mx-auto mt-12 max-w-2xl'} overflow-hidden rounded-3xl border border-slate-700 shadow-2xl shadow-black/20`}>
